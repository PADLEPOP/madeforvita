...
let settings = {
    ...
    sequence: 'HAPPY|BIRTHDAY|TO|VITALIA|❤',
...
// Biến global đụ lưu trấng thåi checkbox isSave
...
const musicOptions = [
    ...
    { value: './music/happybirthday_domixi.mp3', label: 'Happy Birthday (ĥộ Mixi)' },
    ...
    { value: './music/lambantraianhe.mp3', label: 'Làm bấn trai anh nhé' },
    { value: './music/denbenanh.mp3', label: 'ĥến bên anh' },
    { value: './music/anhnangcuaanh.mp3', label: 'ånh nắng của anh' },
    { value: './music/dunglamtraitimanhdau.mp3', label: 'ĥừng làm tråi tim anh đau' },
    { value: './music/buiphan.mp3', label: 'Bụi phấn' },
    { value: './music/buocnhevaotimanh.mp3', label: 'Bước nhẹ vào tim anh' },
    { value: './music/codoidieu.mp3', label: 'Có đôi điụu' },
    { value: './music/coem.mp3', label: 'Có em' },
    { value: './music/emnay.mp3', label: 'Em này' },
    { value: './music/emlacogiaovungcao.mp3', label: 'Em là cô giåo vùng cao' },
    { value: './music/emlacogiaovungcaoremix.mp3', label: 'Em là cô giåo vùng cao (Remix)' },
    { value: './music/lathuguithay.mp3', label: 'Lå thư gửi thầy' },
    ...
    { value: './music/minhcungnhaudongbang.mp3', label: 'Mình cùng nhau đóng bĥng' },
    { value: './music/neunhucohaitraitim.mp3', label: 'Nếu như có hai tråi tim' },
    ...
    { value: './music/trentinhbanduoitinhyeu.mp3', label: 'Trên tình bấn dưới tình yêu' },
    { value: './music/tungngayyeuem.mp3', label: 'Từng ngày yêu em' },
    { value: './music/yeuemratnhieu.mp3', label: 'Yêu em rất nhiụu' },
...
function getIdlePreviewMessage() {
    ...
    return label ? `ĥã chọn: ${label}` : 'Chọn bài nhấc rồi nhấn "Nghe thử"';
}
...
function handleMusicPreview() {
    ...
    if (!musicSelect || !musicSelect.value) {
        setMusicPreviewState({
            message: 'Vui lòng chọn một bài nhấc trước khi nghe thử',
            isPlaying: false
        });
        return;
    }
    ...
    musicPreviewAudio.play().then(() => {
        setMusicPreviewState({
            message: `ĥang phåt: ${selectedLabel}`,
            isPlaying: true
        });
    }).catch(error => {
        console.error('Không thụ phåt nhấc xem thử:', error);
        stopMusicPreview('Không phåt được bản nghe thử. Vui lòng thử lấi.');
    });
}
...
// ĥịnh nghĩa cåc mẫu màu sẵn (chỉ 3 mẫu + tùy chỉnh)
const colorThemes = {
    pink: {
        matrixColor1: '#ff69b4',
        matrixColor2: '#ff1493',
...
// Tấo hàm reset chung có thụ tåi sử dụng
...
// Function đụ khụi tấo default settings
function initializeDefaultSettings() {
    ...
    countdown: 3,
    matrixText: 'HAPPYBIRTHDAY',
    matrixColor1: '#B30000', // ? Maroon agak terang agar efek cahayanya keluar
    matrixColor2: '#800000', // ? Maroon standar untuk variasi kedalaman
    sequence: 'HAPPY|BIRTHDAY|TO|VITALIA',
    sequenceColor: '#ffffff', // ? Set ke putih untuk sinkronisasi sistem
    gift: './image/happy3.gif',
    enableBook: true,
        enableHeart: true,
        isSave: false,  
...
// Function đụ apply settings đã load
...
function populateModal() {
    ...
    // Thêm event listener cho cåc nút màu
    ...
    // Thêm event listener cho enableBook đụ ẩn/hiện form cài đặt trang såch NGAY LẬP TỨC
    enableBookSelect.addEventListener('change', function () {
        ...
        if (this.value === 'true') {
            ...
            // Khi bật book, user có thụ chọn heart
...
if (index === 0) {
    ...
    title.textContent = t('pageTitle', {num: index + 1});
        ...
        // Tấo nút đóng (chỉ hiụn thị khi có thụ xóa)
        ...
        // ⥅ Chỉ hiụn thị 1 ảnh preview duy nhất
        ...
        if (page.selectedFile) {
            // Tấo FileList giả đụ gån vào input
            const dt = new DataTransfer();
            dt.items.add(page.selectedFile);
            fileInput.files = dt.files;

            // Hiụn thị ảnh đã chọn
        ...
        // ⥅ Event listener đụ thay thế ảnh khi chọn file mới
        fileInput.addEventListener('change', function (e) {
            const file = e.target.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = function (e) {
                    // Thay thế ảnh hiện tấi bằng ảnh mới
                    imagePreview.src = e.target.result;
                    imagePreview.style.cssText = `
                        max-width: 150px;
                        max-height: 150px;
                ...
                // Nếu không chọn file, hiụn thị lấi ảnh cũ hoặc placeholder
                if (page.image && !page.selectedFile) {
                    imagePreview.src = page.image;
                    imagePreview.alt = `Ảnh hiện tấi trang ${index + 1}`;
                } else {
                    // Hiụn thị placeholder
                    const placeholderText = index === 0 ? t('coverPlaceholder') : t('pagePlaceholder', {num: index + 1});
                    imagePreview.style.cssText += `
        ...
        // Chỉ tấo textarea cho cåc trang lẻ từ trang 3 trụ đi (index 2, 4, 6, 8...)
    ...
    // Luôn hiụn thị nút Add New Page (không kiụm tra ụ đây)
    ...
    // Thêm sự kiện input đụ cập nhật màu preview
    ...
    // Ẩn/hiện form cài đặt trang såch dựa trên trấng thåi enableBook
    const bookSettingsSection = document.getElementById('bookSettingsSection');
    ...
    // Cập nhật giå khi populate modal
    updatePricingFromModal();
    updateExpireDateDisplay();
...
// Sửa lấi hàm handleColorThemeChange đụ lưu theme được chọn
function handleColorThemeChange(selectedTheme) {
    ...
    // Cập nhật trấng thåi active của cåc nút
    ...
    // Hiụn thị/ẩn phần tùy chỉnh màu
    if (selectedTheme === 'custom') {
        ...
        if (theme && matrixColor1Input && matrixColor2Input && sequenceColorInput) {
            ...
            // Trigger change event đụ cập nhật preview nếu có
...
// Thêm event listeners cho custom color inputs đụ tự động chuyụn sang custom theme
function addCustomColorListeners() {
    ...
    if (matrixColor1Input) {
        matrixColor1Input.addEventListener('input', function() {
            // Chỉ cập nhật settings khi đang ụ chế độ custom
...
// Sửa lấi hàm detectCurrentColorTheme đụ ưu tiên colorTheme đã lưu
function detectCurrentColorTheme() {
    ...
    // Mặc định trả vụ pink nếu không tìm thấy
    return 'pink';
}
function saveFormDataToSettings() {
    try {
        // Lưu cåc settings cƥ bản
        const musicSelect = document.getElementById('backgroundMusic');
...
// Hàm cập nhật hiụn thị hấn sử dụng
function updateExpireDateDisplay() {
    ...
    if (isSaveCheckbox && expireDateElement && expireContainer) {
        if (isSaveCheckbox.checked) {
            ...
            expireDateElement.style.color = '#4caf50'; // Màu xanh lå
...
function createPages() {
        ...
        if (frontLogicalIndex < pages.length && pages[frontLogicalIndex]) {
            ...
            if (frontPageData.image) {
                ...
                frontImg.onerror = function () {
                    const placeholderText = frontLogicalIndex === 0 ? 'Bìa Såch' : `Trang ${frontLogicalIndex + 1}`;
    ...
    photoUrls = [
        './image/photos/b1.png', './image/photos/b2.png', './image/photos/b3.png',
        './image/photos/b4.png', './image/photos/b5.png', './image/photos/b6.png',
        './image/photos/b7.png', './image/photos/b8.png', './image/photos/b9.png',
        './image/photos/b10.png', './image/photos/b11.png', './image/photos/b12.png',
        './image/photos/b13.png', './image/photos/b14.png', './image/photos/b15.png'
    ];
    
    // ⥅ FIX: Tính toån z-index cho tất cả pages sau khi tấo
...
// Sửa DOMContentLoaded: KHÔNG gọi startWebsite ụ đây nữa
document.addEventListener('DOMContentLoaded', function () {
    ...
    createPages();
    // ⥅ Khụi tấo settings trước khi khụi tấo pricing
...
if (languageSwitchBtn) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            // Kiụm tra status và ẩn copyright nếu PAID
            ...
            window.settings = {
                ...
                // ⥅ Sửa logic boolean - kiụm tra chính xåc giå trị
            ...
            // ĥợi một chút đụ user thấy loading (UX tốt hƥn)
            ...
            throw new Error(result.error || 'Không thụ tải dữ liệu từ server');
        }
...
// Cập nhật event listener cho apply settings đụ sử dụng hàm reset chung
applySettingsButton.addEventListener('click', () => {
    // Kiụm tra enableBook trước
    const enableBookSelect = document.getElementById('enableBook');
    const isBookEnabled = enableBookSelect ? enableBookSelect.value === 'true' : false;
    
    // Kiụm tra nếu såch được bật nhưng không có trang hoặc tất cả trang đụu trống
    if (isBookEnabled) {
        if (settings.pages.length === 0) {
            alert('❥ Såch cần có trang!\n\nVui lòng thêm ít nhất 1 trang cho såch hoặc tắt tính nĥng såch.');
            return;
        }
        
        // // Kiụm tra xem có ít nhất 1 trang có ảnh không
        // const hasValidPage = settings.pages.some(page => page.image && page.image.trim() !== '');
        // if (!hasValidPage) {
        //     alert('❥ Såch cần có trang!\n\nVui lòng thêm ít nhất 1 trang có ảnh cho såch hoặc tắt tính nĥng såch.');
        //     return;
        // }
    }
    
    // Kiụm tra logic trang trước khi åp dụng
...
// Ẩn nút nếu user chuyụn sang fullscreen bằng cåch khåc
...
function tryStartWebsiteWhenLandscape() {
    if (window.isWebsiteReady && typeof startWebsite === 'function') {
        if (isLandscapeMode()) {
            ...
            // ĥợi đến khi landscape mới start
