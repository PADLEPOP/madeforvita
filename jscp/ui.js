...
// ⥅ FIX 1: Thêm function đụ force resize matrix canvas
...
function checkOrientation() {
    ...
    if (!isMobile) {
        ...
        if (isLandscape) {
            ...
            // ⥅ FIX 2: Force resize matrix khi chuyụn sang landscape
            setTimeout(() => {
                forceResizeMatrix();
            }, 100);
        } else {
...
function initMatrixRain() {
    ...
    // ĥiụu chỉnh fontSize và tốc độ cho mobile
    const fontSize = isMobile ? 13 : 25;
    const intervalTime = isMobile ? 44 : 50; // mobile chấy chậm hƥn
...
S.Drawing = (function () {
    ...
    return {
        ...
        getArea: function () {
            // ⥅ Kiụm tra canvas trước khi truy cập
            if (!canvas) {
...
// Cập nhật S.Dot với kích thước nhỏ hƥn
S.Dot = function (x, y) {
    this.p = new S.Point({
        x: x,
        y: y,
...
S.Dot.prototype = {
    // Thêm method đụ tính kích thước dot dựa trên thiết bị
    getDotSize: function () {
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

        if (isMobile) {
            return 2; // Dots nhỏ hƥn cho mobile
        } else {
    ...
    _draw: function () {
        // Cập nhật màu theo settings hiện tấi mỗi khi vẽ
        const currentSettings = window.settings || settings;
        const rgb = hexToRgb(currentSettings.sequenceColor);
        this.c.r = rgb.r;
        this.c.g = rgb.g;
...
S.ShapeBuilder = (function () {
    ...
    // ĥiụu chỉnh gap dựa trên thiết bị
    function getGap() {
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

        if (isMobile) {
            return 4; // Gap nhỏ hƥn cho mobile = dots dày đặc hƥn
        } else {
    ...
    function processCanvas() {
        ...
        // Sử dụng gap động đụ tấo nhiụu dots hƥn
...
// Cập nhật S.Shape với logic tấo dots tối ưu
S.Shape = (function () {
    var dots = [],
        width = 0,
        height = 0,
    ...
    return {
        switchShape: function (n, fast) {
            ...
            while (n.dots.length > 0) {
                ...
                const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
                dots[d].e = isMobile ? 0.35 : 0.11; // Tĥng tốc độ di chuyụn cho mobile
...
// 5. Tối ưu hóa hiệu ứng mụ såch
function showBook() {

    const book = document.getElementById('book');
    const bookContainer = document.querySelector('.book-container');

    // Hiụn thị sao một lần
    showStars();
    if (book && bookContainer) {
        ...
        // ⥅ FIX: Tính toån z-index cho tất cả pages
        calculatePageZIndexes();
        
        // ⥅ FIX: Thiết lập observer đụ theo dõi thay đổi cấu trúc book
        setupPageObserver();
...
function showConfetti() {
    ...
    function spawnConfetti() {
        ...
        // Sử dụng setTimeout tối ưu hƥn
        setTimeout(() => returnConfettiToPool(confetti), 1000);

        confettiCount++;

        // Spawn tiếp theo với delay nhỏ hƥn
...
function showFirework() {
    ...
    // Xóa nội dung cũ một cåch hiệu quả
    fireworkContainer.innerHTML = '';
    fireworkContainer.style.opacity = 1;

    // Sử dụng DocumentFragment đụ tối ưu DOM manipulation
    const fragment = document.createDocumentFragment();

    for (let i = 0; i < 20; i++) { // Giảm từ 30 xuống 20
        const fw = document.createElement('div');
        fw.className = 'firework';
        fw.style.transform = `rotate(${i * 18}deg) translateY(-40px)`; // Tĥng góc đụ bù trừ việc giảm số lượng
...
// 3. Tối ưu hóa hiệu ứng tråi tim ảnh với throttling
...
function createHeartPhotoCentered(idx, total) {
    ...
    // --- LOGIKA BARU: EFEK MELAYANG DI ANGKASA ---
    // 1. Sebarkan posisi horizontal (X) secara acak dari kiri ke kanan layar
    const randomX = Math.random() * (window.innerWidth - 100); 
    
    // 2. Sebarkan posisi vertikal (Y) secara acak dari atas ke bawah layar
    const randomY = Math.random() * (window.innerHeight - 100);

    // Tetapkan posisi awal tepat di tengah layar saat pertama kali muncul
    photo.style.left = '50%';
    photo.style.top = '50%';
    
    // Berikan sedikit rotasi acak agar foto terlihat estetik dan tidak kaku
    const randomRotate = (Math.random() * 30) - 15; // antara -15deg sampai 15deg

    // Atur jeda kemunculan beruntun agar foto muncul satu per satu secara halus
    const delay = idx * 250; 

    setTimeout(() => {
        document.body.appendChild(photo);
        
        // Memicu animasi perubahan posisi ke titik melayang acak di angkasa
        requestAnimationFrame(() => {
            photo.style.opacity = '1';
            photo.style.pointerEvents = 'auto';
            
            // Pindahkan ke posisi angkasa barunya secara perlahan
            photo.style.left = `${randomX}px`;
            photo.style.top = `${randomY}px`;
            photo.style.transform = `translate(0, 0) rotate(${randomRotate}deg) scale(1)`;
            
            // Tambahkan animasi tambahan mengambang naik-turun (floating smooth)
            photo.style.animation = `floatingSpace ${5 + Math.random() * 4}s ease-in-out infinite alternate`;
        });
...
function spawnHeartPhotosCentered() {
    ...
    // Preload t?t c? ?nh tr??c
    photoUrls.forEach(url => preloadPhoto(url));

    photoUrls.forEach(url => preloadPhoto(url));
    // ... sisa kode di bawahnya biarkan utuh ...
    // Preload tất cả ảnh trước
    photoUrls.forEach(url => preloadPhoto(url));

    // Sử dụng requestAnimationFrame đụ tấo animation mượt mà hƥn
    let currentIndex = 0;
    function spawnNext() {
...
function startHeartEffect() {
    ...
    // Stagger effects đụ trånh blocking
    requestAnimationFrame(() => {
        setTimeout(() => {
            showConfetti();
        }, 100);
    ...
    return new Promise((resolve) => {
        ...
        function type() {
            if (i < text.length) {
                ...
                // Sử dụng requestAnimationFrame thay vì setTimeout khi có thụ
                if (speed < 16) {
...
function createStars() {
    if (starsCreated) return; // Trånh tấo lấi stars
...
// 9. Cleanup function đụ giải phóng memory
function cleanup() {
    // Clear timeouts
    if (typewriterTimeout) {
        clearTimeout(typewriterTimeout);
...
// ⥅ FIX: Thêm cåc hàm quản lý z-index động cho pages
...
function calculatePageZIndexes() {
    ...
    pages.forEach((page, physicalIndex) => {
        ...
        // Tính toån z-index cho trang hiện tấi
...
function spawnFloatingFlowers() {
    const flowerSymbols = ['\uD83C\uDF38', '\uD83C\uDF39']; // ?? dan ??
    const totalFlowers = 120; // Dinaikkan drastis ke 120 agar background penuh padat!
    ...
    for (let i = 0; i < totalFlowers; i++) {
        ...
        // Sebaran acak dari kiri ke kanan layar
        flower.style.left = `${Math.random() * 100}vw`;
        
        // Posisi awal di bawah layar
        flower.style.top = `${100 + Math.random() * 15}vh`;

        // Ukuran bervariasi gila-gilaan agar efek kedalamannya heboh
        const randomSize = 12 + Math.random() * 24; // 12px hingga 36px
        flower.style.fontSize = `${randomSize}px`;

        // DURASI DIPERCEPAT: Terbang kilat antara 2.5 sampai 5 detik saja agar terasa kencang!
        const duration = 2.5 + Math.random() * 2.5; 
        const delay = Math.random() * 6; // Rilis bersusulan sepanjang 6 detik ke depan
